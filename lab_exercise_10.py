...
def create_thinned_article_tuples(articles):
    ...
    # new_list = []
    # for article in articles:
    #     new_list.append((article['headline_main'], article['pub_date'.split('T')[0], article['word_count']))
...
def get_keywords_by_name(article, keyword_name):
    ...
    #     if i['name'] == keyword_name:
    #         return i['value']

    return [i['value'] for i in article['keywords'] if i['name'] == keyword_name]
...
def sum_word_counts(article_tuples):
    ...
    #     word_counts.append(article[-1])
    # sum_counts = sum(word_counts)

    return sum([article[-1] for article in article_tuples])
